...
(defn single-plate
  ...
  (let [switch-type         (get c :configuration-switch-type)
      ...
      nub-height           (case switch-type
        ...
        ; for the main axis
        main-axis-hole      (->> (cylinder (/ 4.2 2) 12)
                                 (with-fn 12))
        main-axis-hole1      (->> (cylinder 3 8)
                                 (with-fn 12)
                                 (translate [0 0 -4.85]))
        plus-hole           (->> (cylinder (/ 3.5 2) 5)
                                 (with-fn 8)
                                 (translate (if use-choc? [-5 4 0] [-3.81 2.54 0])))
        minus-hole          (->> (cylinder (/ 3.5 2) 5)
                                 (with-fn 8)
                                 (translate (if use-choc? [0 6 0] [2.54 5.08 0])))
        plus-hole-mirrored  (->> (cylinder (/ 3.5 2) 5)
                                 (with-fn 8)
                                 (translate (if use-choc? [5 4 0] [3.81 2.54 0])))
        minus-hole-mirrored (->> (cylinder (/ 3.5 2) 5)
                                 (with-fn 8)
                                 (translate (if use-choc? [0 6 5] [-2.54 5.08 0])))
        friction-hole       (->> (cylinder (if use-choc? 1 (/ 1.7 2)) 10)
                                 (with-fn 8))
        friction-hole-right (translate [(if use-choc? 5.6 5) 0 0] friction-hole)
        friction-hole-left  (translate [(if use-choc? -5.6 -5) 0 0] friction-hole)
